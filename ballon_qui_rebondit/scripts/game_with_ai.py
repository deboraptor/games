...
def move_ball():
    ...
    if ballrect.left < 0 or ballrect.right > width:
        speed[0] = -speed[0]
    if ballrect.top < 0:
        show_messagebox("Vous avez gagné !", win=True)
    if ballrect.bottom > height:
        show_messagebox("L'IA a gagné.")
...
def move_ai():
    if ballrect.centerx > player2.centerx:
        player2.x += player_speed // 2
    elif ballrect.centerx < player2.centerx:
        player2.x -= player_speed // 2
